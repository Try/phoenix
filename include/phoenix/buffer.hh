...
namespace phoenix {
	...
	class BufferUnderflowError : public BufferError {
	public:
		PHOENIX_API BufferUnderflowError(std::uint64_t byte, std::uint64_t size);
		PHOENIX_API BufferUnderflowError(std::uint64_t byte, std::uint64_t size, std::string&& context);
		PHOENIX_API BufferUnderflowError(std::uint64_t byte, std::string&& context);
	...
	public:
		PHOENIX_API BufferOverflowError(std::uint64_t byte, std::uint64_t size);
		PHOENIX_API BufferOverflowError(std::uint64_t byte, std::uint64_t size, std::string&& context);
	...
	class BufferReadonlyError : public BufferError {
	public:
		PHOENIX_API explicit BufferReadonlyError() : BufferError("buffer is not readonly") {}
	};
	...
	class BufferBacking {
	public:
		PHOENIX_API virtual ~BufferBacking() = default;
		...
		/// \throws BufferUnderflowError if filling \p buf with bytes starting at \p offset fails.
		...
		/// \throws BufferOverflowError if writing all bytes of \p buf starting at \p offset fails.
		/// \throws BufferReadonlyError if this backing is readonly.
	...
	class Buffer {
	private:
		PHOENIX_INTERNAL Buffer(std::shared_ptr<BufferBacking> backing, std::uint64_t begin, std::uint64_t end);
		PHOENIX_INTERNAL Buffer(std::shared_ptr<BufferBacking> backing,
		                        std::uint64_t begin,
		                        std::uint64_t end,
		...
		/// \throws BufferUnderflowError if there are not enough bytes #remaining to read the value.
		...
		/// \tparam T The type of scalar to read. Must be a std::integral or a std::floating_point type.
		/// \return The value read.
		/// \throws BufferUnderflowError if there are not enough bytes in the backing to read the value.
		/// \see buffer_backing::read
		template <
		    typename T,
		    typename = typename std::enable_if<std::is_integral<T>::value || std::is_floating_point<T>::value>::type>
		...
		/// \throws BufferOverflowError if there are not enough bytes #remaining to write the value.
	...
	public:
		...
		PHOENIX_API explicit Buffer(std::shared_ptr<BufferBacking> backing);
		...
		/// \throws BufferUnderflowError if \p pos is greater than #limit.
		...
		/// \throws BufferUnderflowError if \p limit is greater than #capacity.
		...
		/// \throws BufferUnderflowError if #position + \p count > #limit
		/// \see #position
		...
		/// \return The newly created buffer.
		/// \throws BufferUnderflowError if \p index + \p size > #limit.
		...
		/// \return The newly created buffer.
		/// \throws BufferUnderflowError if #position + \p size > #limit.
		...
		/// \throws BufferUnderflowError if the size of \p buf > #remaining.
		...
		/// \throws BufferUnderflowError if the value can't be read.
		...
		/// \throws BufferUnderflowError if the string can't be read.
	...
	private:
		...
		std::shared_ptr<BufferBacking> _m_backing;
	...
	using buffer_error PHOENIX_DEPRECATED("renamed to BufferError") = BufferError;
	using buffer_overflow PHOENIX_DEPRECATED("renamed to BufferOverflowError") = BufferOverflowError;
	using buffer_underflow PHOENIX_DEPRECATED("renamed to BufferUnderflowError") = BufferUnderflowError;
	using buffer_readonly PHOENIX_DEPRECATED("renamed to BufferReadonlyError") = BufferReadonlyError;
	using buffer_backing PHOENIX_DEPRECATED("renamed to BufferBacking") = BufferBacking;
	using buffer PHOENIX_DEPRECATED("renamed to Buffer") = Buffer;
} // namespace phoenix
