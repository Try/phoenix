	...
	bool DaedalusVm::exec() {
		...
		try {
			...
			case DaedalusOpcode::MOVS: {
				...
				if (target->is_const() && !(_m_flags & DaedalusVmExecutionFlag::IGNORE_CONST_SPECIFIER)) {
					throw DaedalusIllegalConstAccess(target);
				}
			...
			_m_pc += instr.size;
		} catch (phoenix::DaedalusScriptError& err) {
			...
			if (_m_exception_handler) {
				...
				if (strategy == DaedalusVmExceptionStrategy::FAIL) {
					phoenix::Logging::log(phoenix::LogLevel::ERROR,
...
#pragma GCC diagnostic pop
	...
	void DaedalusVm::print_stack_trace() const {
		...
		log::log(LogLevel::ERROR, "------- CALL STACK (MOST RECENT CALL FIRST) -------");

		while (!callstack.empty()) {
			auto v = callstack.top();
			log::log(LogLevel::ERROR, "in ", v.function->name(), " at 0x", std::hex, last_pc, std::dec);

			last_pc = v.program_counter;
			callstack.pop();
		}

		log::log(LogLevel::ERROR, "------- STACK (MOST RECENT PUSH FIRST) -------");
		...
		while (tmp_stack_ptr > 0) {
			...
			if (v.reference) {
				...
				if (std::holds_alternative<float>(v.value)) {
					log::log(LogLevel::ERROR, tmp_stack_ptr, ": [IMMEDIATE FLOAT] = ", std::get<float>(v.value));
				} else if (std::holds_alternative<int32_t>(v.value)) {
					log::log(LogLevel::ERROR, tmp_stack_ptr, ": [IMMEDIATE INT] = ", std::get<int32_t>(v.value));
				} else if (std::holds_alternative<std::shared_ptr<DaedalusInstance>>(v.value)) {
					auto& inst = std::get<std::shared_ptr<DaedalusInstance>>(v.value);
					if (inst == nullptr) {
						log::log(LogLevel::ERROR, tmp_stack_ptr, ": [IMMEDIATE INSTANCE] = NULL ");
					} else {
						log::log(LogLevel::ERROR,
						         tmp_stack_ptr,
