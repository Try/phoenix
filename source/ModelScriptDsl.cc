...
namespace phoenix {
	...
	MdsTokenizer::MdsTokenizer(Buffer buf) : _m_buffer(std::move(buf)) {}
	...
	MdsToken MdsTokenizer::next() {
		...
		while (_m_buffer.remaining() > 0) {
			...
			if (std::isdigit(chr) || chr == '-') {
				...
				return floating_point ? MdsToken::float_ : MdsToken::integer;
			}
			...
			WARN_SYNTAX("unknown MdsToken \"", chr, "\"");
			return MdsToken::null;
		}
	...
	MdsParser::MdsParser(Buffer buf) : _m_stream(std::move(buf)) {}
	...
	std::optional<float> MdsParser::maybe_number() {
		...
		if (n != MdsToken::integer && n != MdsToken::float_) {
	...
	ModelScript MdsParser::parse_script() {
		ModelScript script {};
		...
		while (!this->eof()) {
			if (this->maybe<MdsToken::rbrace>() || this->maybe<MdsToken::eof>()) {
				break;
			}
			...
			if (phoenix::iequals(kw, "meshAndTree")) {
				...
				PX_LOGW("ModelScript: detected invalid use of keyword " + kw +
